...
import type { AddEmployeeFormFieldValues, AddEmployeeFormProperties } from "types/forms"

export const AddEmployeeForm = ({ messages }: AddEmployeeFormProperties) => {
	const defaultValues: Partial<AddEmployeeFormFieldValues> = {}

	const form = useForm<AddEmployeeFormFieldValues>({
		defaultValues,
		mode: "onSubmit",
	...
	const handleOnSubmit: SubmitHandler<AddEmployeeFormFieldValues> = async (data) => {
	...
	return (
		<form className="w-full" onSubmit={form.handleSubmit(handleOnSubmit)}>
			<Card className="flex flex-col">
				<CardHeader>
					<CardTitle>{messages?.forms?.addEmployee.title}</CardTitle>
					<CardDescription>{messages?.forms?.addEmployee.subtitle}</CardDescription>
				</CardHeader>
